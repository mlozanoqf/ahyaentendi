...
$$
con objetivos distintos: (1) estimar $\hat\beta$ para cuantificar exposición al mercado; (2) derivar un rendimiento requerido del equity $k_e = R_f+\hat\beta\cdot ERP$; y (3) evaluar desempeño de un fondo con el alpha de Jensen $\hat\alpha$.
...
\begin{itemize}
\item Interpretación de $\widehat{\beta}$: la sensibilidad estimada es mayor que uno. En la muestra, \texttt{AAPL} reacciona más que proporcionalmente a variaciones del S\&P 500 en exceso de la tasa libre de riesgo. En términos marginales, un cambio de $1\%$ en $r_{M,t}$ se asocia con un cambio de $`r fmt2(coef(fit_aapl)[["mkt_excess"]])`\%$ en $r_{i,t}$.
...
\widehat{\beta}\,r_M.
\]
Para un escenario $r_M=+5\%$, $\widehat{\beta}\,r_M = `r fmt2(coef(fit_aapl)[["mkt_excess"]])`\times 5\% = `r fmt2(coef(fit_aapl)[["mkt_excess"]] * 5)`\%$. Para $r_M=-5\%$, el componente es $`r fmt2(-coef(fit_aapl)[["mkt_excess"]] * 5)`\%$.
...
\]
donde $r_M$ se interpreta como un escenario para el rendimiento del mercado en exceso. Para $r_M=+5\%$, $\widehat r_{i,t} = `r fmt2(coef(fit_aapl)[["(Intercept)"]])` + `r fmt2(coef(fit_aapl)[["mkt_excess"]])`\times 5\% = `r fmt2(coef(fit_aapl)[["(Intercept)"]] + coef(fit_aapl)[["mkt_excess"]] * 5)`\%$. Para $r_M=-5\%$, $\widehat r_{i,t} = `r fmt2(coef(fit_aapl)[["(Intercept)"]] - coef(fit_aapl)[["mkt_excess"]] * 5)`\%$.
...
\item Resultados en escenarios (HAC, 95\%). Para $r_M=-5\%$, el modelo implica $\widehat r_{i,t} = `r fmt2(ci_mean_df$fit[ci_mean_df$rM == -5])`\%$ y un intervalo al 95\% de [$`r fmt2(ci_mean_df$lwr[ci_mean_df$rM == -5])`\%$, $`r fmt2(ci_mean_df$upr[ci_mean_df$rM == -5])`\%$], con EE HAC $= `r fmt2(ci_mean_df$se[ci_mean_df$rM == -5])`$. Para $r_M=+5\%$, $\widehat r_{i,t} = `r fmt2(ci_mean_df$fit[ci_mean_df$rM == 5])`\%$ y el intervalo al 95\% es [$`r fmt2(ci_mean_df$lwr[ci_mean_df$rM == 5])`\%$, $`r fmt2(ci_mean_df$upr[ci_mean_df$rM == 5])`\%$], con EE HAC $= `r fmt2(ci_mean_df$se[ci_mean_df$rM == 5])`$.
...
\item Por construcción, el intervalo de predicción es sustancialmente más amplio que el intervalo para la media condicional: además de la incertidumbre sobre $(\widehat{\alpha},\widehat{\beta})$, incorpora variación idiosincrática mensual de \texttt{AAPL} que el mercado no captura. En los escenarios considerados, el error estándar de la media condicional es relativamente pequeño, con $\widehat{se}_{mean}= `r fmt2(pi_df$se_mean[pi_df$rM == -5])`$ para $r_M=-5\%$ y $\widehat{se}_{mean}= `r fmt2(pi_df$se_mean[pi_df$rM == 5])`$ para $r_M=+5\%$, mientras que el error estándar de predicción es $\widehat{se}_{pred}= `r fmt2(pi_df$se_pred[pi_df$rM == -5])`$ y $\widehat{se}_{pred}= `r fmt2(pi_df$se_pred[pi_df$rM == 5])`$, reflejando que la componente residual domina la dispersión mensual.

\item Resultados en escenarios (predicción, 95\%). Para $r_M=-5\%$, el valor central del modelo es $\widehat r_{i,t} = `r fmt2(pi_df$fit[pi_df$rM == -5])`\%$ y el intervalo de predicción al 95\% es [$`r fmt2(pi_df$pred_lwr[pi_df$rM == -5])`\%$, $`r fmt2(pi_df$pred_upr[pi_df$rM == -5])`\%$]. Para $r_M=+5\%$, $\widehat r_{i,t} = `r fmt2(pi_df$fit[pi_df$rM == 5])`\%$ y el intervalo de predicción es [$`r fmt2(pi_df$pred_lwr[pi_df$rM == 5])`\%$, $`r fmt2(pi_df$pred_upr[pi_df$rM == 5])`\%$].
...
\[
k_e = R_f + \widehat{\beta}_{KO}\cdot ERP,
\]
...
$\widehat{\alpha}_{KO}=`r fmt2(ko_ols$estimate[ko_ols$term == "alpha"])`,\quad SE=`r fmt2(ko_ols$std.error[ko_ols$term == "alpha"])`,\; t=`r fmt2(ko_ols$statistic[ko_ols$term == "alpha"])`,\; p=`r fmt_p(ko_ols$p.value[ko_ols$term == "alpha"])`$,

$\widehat{\beta}_{KO}=`r fmt2(ko_ols$estimate[ko_ols$term == "beta"])`,\quad SE=`r fmt2(ko_ols$std.error[ko_ols$term == "beta"])`,\; t=`r fmt2(ko_ols$statistic[ko_ols$term == "beta"])`,\; p=`r fmt_p(ko_ols$p.value[ko_ols$term == "beta"])`$.

\item Lectura operativa: $\widehat{\beta}_{KO}=`r fmt2(ko_ols$estimate[ko_ols$term == "beta"])`$ implica que, en esta ventana y con el S\&P 500 como benchmark, \texttt{KO} exhibe una exposición al riesgo de mercado inferior a la unidad. Este es el insumo central para convertir prima de mercado esperada en un rendimiento requerido para el equity.
...
\item Transición: en el siguiente bloque calculamos $k_e = R_f+\widehat{\beta}_{KO}\cdot ERP$, reportando un caso base y sensibilidad ante distintos supuestos de $ERP$.
\end{itemize}
...
k_e = R_f + \widehat{\beta}_{KO}\cdot ERP.
\]
Aquí $R_f$ se anualiza a partir del promedio mensual observado en la ventana. En tu muestra, el promedio anualizado es $R_f = `r fmt2(rf_ann)`\%$.

\item Resultados (caso base y sensibilidad): con $\widehat{\beta}_{KO}=`r fmt2(beta_ko_hat)`$, el costo de equity queda:
\[
k_e = `r fmt2(rf_ann)`\% + `r fmt2(beta_ko_hat)`\times ERP.
\]
En particular, si $ERP=4\%$, $k_e=`r fmt2(ke_table$ke_ann[ke_table$ERP_ann == 4])`\%$; si $ERP=5\%$, $k_e=`r fmt2(ke_table$ke_ann[ke_table$ERP_ann == 5])`\%$; si $ERP=6\%$, $k_e=`r fmt2(ke_table$ke_ann[ke_table$ERP_ann == 6])`\%$; si $ERP=7\%$, $k_e=`r fmt2(ke_table$ke_ann[ke_table$ERP_ann == 7])`\%$; y si $ERP=8\%$, $k_e=`r fmt2(ke_table$ke_ann[ke_table$ERP_ann == 8])`\%$.

\item Lectura de sensibilidad: un cambio de 1 punto porcentual en el $ERP$ anual cambia $k_e$ en $\widehat{\beta}_{KO}=`r fmt2(beta_ko_hat)`$ puntos porcentuales. Esta elasticidad es el vínculo operativo entre exposición al mercado y rendimiento requerido del equity.
...
$\widehat{\alpha}_{FDGRX}=`r fmt2(fdgrx_ols$estimate[fdgrx_ols$term == "alpha"])`,\quad SE_{OLS}=`r fmt2(fdgrx_ols$std.error[fdgrx_ols$term == "alpha"])`,\; t=`r fmt2(fdgrx_ols$statistic[fdgrx_ols$term == "alpha"])`,\; p=`r fmt_p(fdgrx_ols$p.value[fdgrx_ols$term == "alpha"])`$,

$SE_{HAC}=`r fmt2(fdgrx_hac$std.error[fdgrx_hac$term == "alpha"])`,\; t_{HAC}=`r fmt2(fdgrx_hac$statistic[fdgrx_hac$term == "alpha"])`,\; p_{HAC}=`r fmt_p(fdgrx_hac$p.value[fdgrx_hac$term == "alpha"])`$.

$\widehat{\beta}_{FDGRX}=`r fmt2(fdgrx_ols$estimate[fdgrx_ols$term == "beta"])`$.
...
\item Lectura de $\widehat{\beta}$: el fondo exhibe una exposición sistemática superior a la del mercado. Con $\widehat{\beta}=`r fmt2(fdgrx_ols$estimate[fdgrx_ols$term == "beta"])`$, un cambio de $1\%$ en el exceso del mercado se asocia, en promedio condicional, con un cambio de `r fmt2(fdgrx_ols$estimate[fdgrx_ols$term == "beta"])`\% en el exceso de \texttt{FDGRX}.
