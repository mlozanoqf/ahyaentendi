...
\textbf{Empresa A}: Tiene liquidez perfecta pero no invierte en activos productivos, así que no genera ventas ni utilidades. 

\textbf{Empresa B}: Reduce liquidez para invertir en activos que producen ingresos. Tiene menos efectivo, pero mayor rentabilidad.
\end{minipage}
\end{flushleft}
...
# Datos balance
balance <- data.frame(
  Balance_General = c("Activos fijos", "Pasivos (deuda)", "Capital"),
  A = c("$1,000", "$0", "$1,000"),
  B = c("$1,000", "$500", "$500")
)
...
estado_resultados <- data.frame(
  Estado_de_Resultados = c("Ventas", "Costos operativos", "Depreciación", "Intereses",
...
# Datos ROE
roe <- data.frame(
  Análisis = c("Utilidad neta", "Capital", "ROE"),
  Sin_deuda = c("$350", "$1,000", "35%"),
  Con_deuda = c("$315", "$500", "63%")
)
...
#  cat("\\textbf{Estado de resultados}\\\\[-0.2em]\n")
  ...
  cat("\\vspace{1em}\n")
    ...
    cat("\\noindent\n")
  cat("\\begin{minipage}[t]{0.49\\textwidth}\n")
#  cat("\\textbf{Rendimiento sobre el capital}\\\\[-0.2em]\n")
  print(tabla_roe)
  cat("\\end{minipage}\n")
