...
subtitle: "Una introducción al diseño y análisis."
...
|--------|------------------------------------|----------------------------------------------------------------|
| AMD    | Advanced Micro Devices, Inc.       | Computación de alto rendimiento              |
...
tibble(symbol = label,
       ER = mean(pr$portfolio_return, na.rm = TRUE),
       SD  = sd(pr$portfolio_return, na.rm = TRUE),
       SR = ER / SD)}
...
stats_all <- tibble(symbol = names(mu_vec_all),
                    ER = mu_vec_all,
...
portfolio_50_50 <- portfolio_series(monthly_returns, 
                                    c(LRCX = 0.5, GIS = 0.5),
...
ggplot(plot_data, aes(date, index_level, color = factor(symbol, levels = last_vals$symbol))) +
  geom_line(linewidth = 1) +
  geom_text(data = label_data, aes(label = label), hjust = -0.05, vjust = 0.5, show.legend = FALSE) +
  scale_color_manual(values = color_values, limits = last_vals$symbol) +
  scale_x_date(expand = expansion(mult = c(0, 0.1))) +
  scale_y_continuous(labels = dollar_format(prefix = "$")) +
  labs(title = "Rendimiento acumulado.", y = "Rendimiento acumulado", x = NULL, color = "Series") +
  theme_minimal() +
...
stats_lrcx_gis <- stats |> filter(symbol %in% names(weights_50_50))
...
ggplot() +
  ...
  scale_color_manual(values = color_values, limits = levels(mean_var_points$symbol)) +
    scale_x_continuous(limits = c(0.025, 0.18), labels = percent_format(accuracy = 0.1)) +
  scale_y_continuous(limits = c(min(mu_vec_all), max(mu_vec_all)), 
                     labels = percent_format(accuracy = 0.1)) +
  labs(title = "Frontera media-varianza.",
...
ggplot() +
  ...
  scale_x_continuous(limits = c(0.025, 0.18), labels = percent_format(accuracy = 0.1)) +
  scale_y_continuous(limits = c(min(mu_vec_all), max(mu_vec_all)), labels = percent_format(accuracy = 0.1)) +
  labs(title = "Frontera media-varianza.",
       x = "Volatilidad", y = "Rendimiento esperado", color = "Series") +
  theme_minimal() +
...
```{r}
minvar10_ns_label <- "MV (no shorts)"

weights_minvar10_ns <- 
  solve.QP(2 * cov_mat_all, rep(0, length(mu_vec_all)),
...
minvar_point_10_ns <- 
  tibble(symbol = minvar10_ns_label,
...
```{r}
minvar_weights_table_ns <- 
  tibble(symbol  = names(mu_vec_all),
...
minvar_point_10_short <- 
  tibble(symbol = minvar10_short_label, ER = sum(weights_minvar10_short * mu_vec_all),
         SD = sqrt(as.numeric(t(weights_minvar10_short) %*% Sigma_all %*% weights_minvar10_short)))

weights_gmv_table_short <- 
  tibble(symbol = names(mu_vec_all),
  ...
  bind_rows(tibble(symbol = "Total",
                   ...
                   weight_pct = scales::percent(sum(weights_minvar10_short), accuracy = 0.01)))
...
ggplot() +
  ...
  scale_x_continuous(limits = c(0.025, 0.19), labels = percent_format(accuracy = 0.1)) +
  scale_y_continuous(limits = c(-0.01, 0.03), labels = percent_format(accuracy = 0.1)) +
  labs(title = "Frontera media-varianza, sin ventas en corto.",
...
stats_all_ext_short <- bind_rows(stats_all, minvar_point_10_short)
color_values_ext_short <- c(color_values_all, setNames("black", minvar10_short_label))
...
ggplot() +
  ...
  scale_y_continuous(limits = c(-0.01, 0.03),
                     labels = percent_format(accuracy = 0.1)) +
  labs(title = "Fronteria media-varianza, con ventas en corto.",
       x = "Volatilidad", y = "Rendimiento esperado") +
  guides(color = "none") +
  theme_minimal()
```
...
weights_specs <- 
  list("50/50 LRCX-GIS" = weights_50_50,
...
```{r}
portfolio_cum <- imap_dfr(weights_specs, ~ portfolio_series(monthly_returns, .x, .y, price_start))
...
palette_base <- c("50/50 LRCX-GIS" = "black",
                  ...
                  "MV 10-Asset (Shorts Allowed)" = "#ff7f0e")
...
label_data_port <- portfolio_cum |>
  group_by(symbol) |> filter(date == max(date)) |>
  mutate(label = paste0("$", formatC(index_level, format="f", digits=2))) |>
  ungroup()
