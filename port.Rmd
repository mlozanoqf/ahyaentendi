...
gis_snps_wide <- monthly_returns |>
  ...
  select(date, symbol, monthly_return) |>
  pivot_wider(names_from = symbol, values_from = monthly_return) |>
  drop_na()   # por si falta algún dato en alguna fecha
...
minvar_point_10_short <- tibble(symbol = minvar10_short_label, 
...
Amat_short <- rbind(rep(1, n_assets), mu_vec_all)
  ...
  ggplot() +
    ...
    scale_x_continuous(limits = c(0.025, 0.19), labels = percent_format(accuracy = 0.1)) +
    scale_y_continuous(limits = c(-0.01, 0.03), labels = percent_format(accuracy = 0.1)) +
    labs(title = "Fronteria media-varianza, 10 activos con ventas en corto.",
         x = "Volatilidad", y = "Rendimiento esperado") +
    guides(color = "none") +
    theme_minimal()
```
