...
$u_i = \frac{S_i - S_{i-1}}{S_{i-1}} \rightarrow u_i = \frac{S_i}{S_{i-1}}-1$.

$\rightarrow u_2 = \frac{2099.60}{2076.62}-1 \rightarrow u_2 = 0.0110660492$.
...
GARCH(1,1): $\sigma^2_n=\omega+\alpha u^2_{n-1}+\beta \sigma^2_{n-1}$.
...
S <- S |> 
  mutate(v_i = case_when(i <= 2 ~ NA_character_,
                         i == 3 ~ "u_i^2",
                         i >= 4 ~ "GARCH(1,1)"))
...
GARCH(1,1): $\sigma^2_n=\omega+\alpha u^2_{n-1}+\beta \sigma^2_{n-1}$, $v_i=\sigma^2_i$.

Función de log-verosimilitud condicional: $\sum^{m}_{i=1} \left[ -ln(v_i)-\frac{u^2_i}{v_i}\right]$.
...
# Estimación Full MLE - Resultados.
...
```{r}
knitr::kable(summary_tbl, align = c("l", "r"), digits = c(0, 6), 
             format.args = list(scientific = FALSE), 
             col.names = c("Estimador / métrica", "Valor"))
```
...
summary_vt <- tibble::tibble(
  metric = c("omega (target)", "alpha", "beta", "logLik", "n_obs", "AIC", "BIC"), 
  value  = c(omega_vt, alpha_vt, beta_vt, ll_vt, n_obs_vt, AIC_vt, BIC_vt))
```

\newpage

# Estimación *variance targeting* - Resultados.
...
```{r}
knitr::kable(summary_vt, align = c("l", "r"), digits = c(0, 6), 
             format.args = list(scientific = FALSE), 
             col.names = c("Estimador / métrica", "Valor"))
```
...
# Visualización de la minimización *variance targeting*.

```{r}
alphas <- seq(0.22, 0.3, length.out = 100)
betas  <- seq(0.66, 0.76, length.out = 100)

ll_surface <- outer(alphas, betas, Vectorize(function(a, b) {
  if (a < 0 || b < 0 || (a + b) >= 0.999) return(NA_real_)
  -nll_garch_VT_shifted(c(a, b), u = u, VL = VL)}))

contour(x = betas, y = alphas, z = t(ll_surface), nlevels = 50,
        xlab = expression(beta), ylab = expression(alpha),
        main = "Contornos de la log-verosimilitud (variance targeting.)")
points(beta_vt, alpha_vt, pch = 19, cex = 2, col = "red")
abline(h = alpha_vt, col = "red", lty = 2)
abline(v = beta_vt, col = "red", lty = 2)
  legend("topright",
    legend = sprintf("alpha = %.4f\nbeta = %.4f", alpha_vt, beta_vt),
    bg = "white", text.col = "black")
```
...
```{r}
y_top <- max(100 * sqrt(S$v_i), na.rm = TRUE) * 0.95
x_left <- min(S$date, na.rm = TRUE) + 50

ggplot(S, aes(x = date, y = 100 * sqrt(v_i))) +
  ...
  annotate("text", x = x_left, y = y_top,
    label = "La volatilidad medida con un GARCH(1,1) tiende a subir rápidamente \n cuando ocurre un shock, pero disminuye de manera más lenta,\n reflejando una cierta persistencia y un proceso de reversión hacia \n la media o hacia su nivel de volatilidad de largo plazo, el típico efecto \n de clustering o agrupamiento de volatilidad observado en los mercados \n financieros.",
...
Azul, GARCH(1,1): $\sigma^2_n=\omega+\alpha u^2_{n-1}+\beta \sigma^2_{n-1}$.

Rojo, Volatilidad no condicional: $s = \frac{1}{n - 1} \sum_{i=1}^{n} (u_i - \bar{u})^2$.
