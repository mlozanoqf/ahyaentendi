...
%     end
 ...
 for j = 1:length(rho);
    ...
    p=bar(w_GMV(i,:));
    axis([0 11 -1.5 2])
 %hleg = legend('Consumer','Manufactures','HiTec','Other');
    text(1,1.8,'ini:','HorizontalAlignment','right')
    text(1,1.6,'end:','HorizontalAlignment','right')
    text(2.2,1.8,num2str(t_s(1)),'HorizontalAlignment','right')
    text(2.2,1.6,num2str(t_s(end)),'HorizontalAlignment','right')
    xlabel ('NoDur  Durbl  Manuf  Enrgy  HiTec  Telcm  Shops  Hlth   Utils  Other')
    ylabel ('Unrestricted weights')
    title ('US minimum variance portfolio composition: 10 industry portfolios')
%     p=plot(sdp,rho, 'r');
