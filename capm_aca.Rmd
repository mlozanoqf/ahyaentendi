...
\item Serie de tiempo: estimamos $r_{p,t}=\alpha_p+\beta_p r_{M,t}+\varepsilon_{p,t}$ para obtener $\hat\beta_p$ (exposición al mercado) y $\hat\alpha_p$ (desvío ex post). Reportamos errores estándar OLS y HAC para inferencia robusta ante heterocedasticidad y dependencia temporal.
...
\begin{itemize}
\item Las $\hat\beta_p$ se ubican entre $\approx 0.94$ y $\approx 1.21$ y son altamente significativas. En general, las carteras de mayor tamaño tienden a mostrar menor exposición al mercado.
...
\begin{itemize}
\item El precio del riesgo $\gamma_1$ es positivo y estadísticamente relevante en la muestra ($\hat\gamma_1\approx 0.95\%$ mensual). El ajuste estilo Shanken incrementa levemente los errores estándar sin alterar el signo ni el mensaje económico.
...
# Visualización de Betas, rendimientos en exceso y linea de regresión.
...
# Visualización de las estimaciones Fama Mac-Beth.
...
\begin{itemize}
\item El precio del riesgo de mercado es positivo en promedio: $\bar\lambda_1 \approx 0.95\%$ mensual, coherente con la sección cruzada. La evidencia es moderada, con $p \approx 0.05$ (SE básicos) y $p \approx 0.07$ (HAC Newey--West), reflejando la variación temporal de $\hat\lambda_{1,t}$.
\item El intercepto promedio es $\bar\lambda_0 \approx -0.18\%$ y no es estadísticamente distinto de cero; la corrección Newey--West aumenta modestamente los errores estándar sin cambiar el mensaje central.
\end{itemize}
...
\begin{itemize}
\item Los datos respaldan la lógica central del CAPM: el rendimiento esperado en exceso aumenta con $\beta$ y el intercepto es compatible con cero.
\item El precio del riesgo estimado es $\bar\lambda_1 \approx 0.95\%$ mensual. Como aproximación lineal, esto equivale a $\approx 11\%$ anualizado; la evidencia estadística es moderada, pero el signo y el orden de magnitud se mantienen bajo correcciones HAC.
\item Las $\alpha_p$ no muestran desvíos sistemáticos. Una vez controlada la compensación por riesgo de mercado vía $\beta$, no queda un excedente sistemático sin explicar.
\item El CAPM luce coherente pero no definitivo: en estas 10 carteras el ajuste es relativamente favorable, en parte porque trabajamos con carteras diversificadas y una muestra larga; aun así, modelos multifactor (tamaño, value, momentum) pueden capturar variación adicional y reducir alphas residuales.
\end{itemize}
